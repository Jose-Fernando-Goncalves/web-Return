...
import { Aperture, Bug, Check, Trash } from 'phosphor-react'
  ...
  async  function handleFeedback(event:FormEvent){
    ...
    return(
        <div className="flex gap-4 w-full h-full flex-col ">
            <div className="text-lime-400 flex-row flex gap-2 justify-center">
              {sended ? <Check size="30" className="mt-0"/> : <Bug size="30" className="mt-1"/>} 
              {sended ? <span className="text-2xl"> Sucesso!</span> :  <span className="text-3xl"> BUG</span>}
            </div>
